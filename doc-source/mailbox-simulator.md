...
Amazon Simple Email Service \(Amazon SES\) provides a mailbox simulator that you can use to test how your application handles various email sending scenarios without affecting your sending quota or your bounce and complaint metrics\. The Amazon SES mailbox simulator is a set of test email addresses\. Each email address represents a specific scenario\. You can send emails to the mailbox simulator when you want to:
...
The mailbox simulator provides typical bounce, complaint, and OOTO responses\. In the bounce scenario, multiple bounces from the same sending request are gathered into a single response\. In practice, the response varies by ISP\. To reduce your bounce and complaint rates, see the [Amazon Simple Email Service Email Sending Best Practices whitepaper](https://media.amazonwebservices.com/AWS_Amazon_SES_Best_Practices.pdf)\.
